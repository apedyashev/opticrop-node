...
var GAMMA, Opticrop, async, gd, gm, utils;
...
Opticrop = (function() {
    ...
    return async.auto({
      ...
      calculateCenter: [
        'size', 'createEdgedImage', (function(_this) {
          return function(cb, results) {
            console.time("pix");
            return _this._createGdImage(results.createEdgedImage.resultFile, function(err, gdImage) {
              ...
              for (k = _j = 0; 0 <= nk ? _j < nk : _j > nk; k = 0 <= nk ? ++_j : --_j) {
                ...
                for (c = _k = 0; 0 <= n ? _k < n : _k > n; c = 0 <= n ? ++_k : --_k) {
                  ...
                  beta += gdImage.getPixel(i, j) & 0xFF;
                }
                area = wcrop * hcrop;
  ...
  Opticrop.prototype._createGdImage = function(inFile, done) {
    return gm(inFile).format(function(err, format) {
      var image;
      if (err) {
        return done("_createGdImage error: " + err);
      }
      switch (format) {
        case "JPEG":
          image = gd.createFromJpeg(inFile);
          break;
        case "PNG":
          image = gd.createFromPng(inFile);
          break;
        case "GIF":
          image = gd.createFromGif(inFile);
          break;
        default:
